...
#H[快速上手][
    ...
    I[生成用户输入模板][
        ...
        I[按`Ctrl+N`、创建一个临时文件],
        I[
          按`Ctrl+Shift+P`, 输入`ManualAi-MkReqTemplate`, 回车执行命令, 这会生成 供用户输入的yaml模板, 并将其写入剪贴板, 然后您需按`Ctrl+V`把模板粘贴到当前临时文件中
    ...
    I[编辑模板][
      ...
      需要在files中添加文件上下文。支持添加绝对路径, 带通配符的绝对路径, 正则表达式匹配文件上下文, 并自动读取带行号的文件内容
      注意: *不支持相对路径!*
    ...
    I[生成请求文本][
      ...
      先把上一步编辑的yaml请求的全文复制到剪贴板中,
      然后执行`ManualAi-MkInitReq`命令、这将生成 面向大模型的请求文本 并 将最终的请求文本写入剪贴板。生成的文本中会带上系统提示词。
    ...
    I[执行大模型操作][
      ...
      *在执行操作之前, 强烈建议您做好版本控制和备份* 如先git commit一次
    ],
  )
]
...
#H[YamlMd 格式][
  ...
  - #link("Prompt/Prompt.typ")[默认系统提示词(typst版)]
  - #link("Prompt/Prompt.md")[默认系统提示词(markdown版, 由typst版转换生成)]
