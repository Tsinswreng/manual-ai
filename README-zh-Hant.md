Manual-AI 是一款 VSCode 擴展。通過結構化的 YamlMd 數據交互, 用戶能利用網頁上的AI大模型, 使大模型與編輯器結合㕥執行修改代碼等操作, 達到近似AI Agent的編程體驗。
...
6. 執行大模型操作 複製下大模型的響應文本到剪貼板，並執行`ManualAi-ExeOp`命令，擴展會解析響應文本，執行對應的寫操作。 **在執行操作之前, 強烈建議您做好版本控制和備份** 如先git commit一次
