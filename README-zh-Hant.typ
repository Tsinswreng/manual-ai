...
#H[快速上手][
    ...
    I[生成用戶輸入模板][
        ...
        I[按`Ctrl+N`、創建一個臨時文件],
        I[
          按`Ctrl+Shift+P`, 輸入`ManualAi-MkReqTemplate`, 回車執行命令, 這會生成用戶輸入yaml的模板並寫入剪貼板, 然後`Ctrl+V`粘貼到當前臨時文件中
          ```yaml
          files:
            paths:
              - ""
    ...
    I[生成請求文本][
      執行`ManualAi-MkInitReq`命令、這將生成面向大模型的請求文本並寫入剪貼板。生成的文本中會帶上系統提示詞。
      默認的系統提示詞會要求大模型以約定的yaml格式影響, 㕥便後續程序解析
    ...
    I[執行大模型操作][
      ...
      網頁版大模型響應格式可能是不合法的yaml語法(比如大模型可能會把yaml包在代碼塊中 或添加額外的描述文本, 儘管我們已經在默認提示詞中強調過這些規則, 但大模型仍可能不聽),此時可能需要您手動修改一些錯誤, 避免解析失敗
