[漢](README-zh-Hant) [汉](README-zh-Hans)
...
3. Edit the Template Example
   ...
   ```
4. Generate Request Text Execute the `ManualAi-MkInitReq` command, which will generate request text for the LLM and copy it to the clipboard. The generated text will include the system prompt. The default system prompt requires the LLM to respond in the agreed YAML format for subsequent program parsing.
